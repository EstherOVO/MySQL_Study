...
-- 셀렉션 컨디션(조건식) : 조건 선택
-- 급여가 1,000 달러 이상인 직원만 조회
SELECT * FROM EMP WHERE SAL > 1000;

-- 셀렉션 + 프로젝션
-- 급여가 1,000 달러 이상이고, 부서 번호가 10번인 직원의 이름만 조회
...
-- IS NOT NULL 비교연산자 : NULL이 아닌 값의 행 선택
SELECT * FROM EMP WHERE COMM IS NOT NULL;

-- BETWEEN .. AND : 특정 범위 내 값 찾기
-- WHERE 컬럼명 BETWEEN 값1 AND 값2;
-- 범위의 시작 값(값1)부터 범위의 끝 값(값2)까지 조회하고, 두 값 모두 포함
-- 1. 날짜 범위
SELECT ENAME, HIREDATE FROM EMP WHERE HIREDATE BETWEEN '1981-01-01' AND '1982-12-31';
...
-- OR 조건 여러 개와 대체될 수 있다.
-- WHERE 컬럼명 IN (값1, 값2, ...)
-- 쿼리문을 간결하게 만들고, 동적으로 여러 조건을 포함해야 할 때 사용
SELECT ENAME, JOB FROM EMP WHERE JOB IN ('MANAGER', 'ANALIST', 'CLERK');
SELECT ENAME, JOB FROM EMP WHERE JOB = 'MANAGER' OR JOB = 'ANALIST' OR JOB = 'CLERK';	-- 위와 동일한 결과
