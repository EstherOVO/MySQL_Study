...
SELECT Language, count(*) FROM countrylanguage GROUP BY Language ORDER BY count(Language) DESC LIMIT 3;
...
SELECT Name, GNP * Population AS GDP FROM country ORDER BY GDP DESC LIMIT 5;
...
SELECT Language, count(*) FROM countrylanguage GROUP BY Language HAVING count(Language) >= 5;
